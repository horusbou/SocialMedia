...
export async function postFollow(req: Request, res: Response) {
	...
	try {
		//@ts-ignore
		const user = await UserModel.findByPk(req.user.dataValues.user_id);
...
export async function postUnfollow(req: Request, res: Response) {
	...
	try {
		...
		const queryBuilder = new QueryBuilder().raw(
			`match (n:User {user_id:'${user.dataValues.user_id}'})-[f:Follows]->(u:User {user_id:'${targetId}'}) delete f`
		);
...
export async function getFollowers(req: Request, res: Response) {
	...
	const following = relationship.map((follow: any) => follow.target.dataValues);
